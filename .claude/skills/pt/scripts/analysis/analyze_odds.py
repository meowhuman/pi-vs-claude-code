...
def analyze_market_with_history(event: Dict, lookback_days: List[int]) -> List[Dict]:
    ...
    for market in markets:
        ...
        # 🟢 修正：使用 get_market_info 獲取最新價格
        print(f"  📊 正在獲取 {question[:30]}... 的最新價格...", end="", flush=True)
        live_market_data = get_market_info(condition_id)
        
        if live_market_data:
            # Use live data for current prices
            outcome_prices_str = live_market_data.get('outcomePrices', market.get('outcomePrices', '["0", "0"]'))
            volume_24h = float(live_market_data.get('volume24hr', market.get('volume24hr', 0)))
            total_volume = float(live_market_data.get('volumeNum', live_market_data.get('volume', market.get('volumeNum', 0))))
            
            # Get best bid/ask for more accurate current price
            best_bid = live_market_data.get('bestBid')
            best_ask = live_market_data.get('bestAsk')
            print(" ✅")
        else:
        ...
        try:
            ...
            # If we have bestBid/bestAsk, use them for more accurate current price
...
def fetch_single_market(condition_id: str) -> Optional[Dict]:
    """獲取單一市場數據 (by condition_id) - 使用 utils/market.py"""
    try:
        print(f"  📊 Fetching market info...")
        
        # Get market info using the existing utility
        ...
        # Get outcome prices - prefer from market_data
        ...
        if outcome_prices:
            ...
            # Try to get from bestBid/bestAsk
            best_bid = market_data.get('bestBid', 0)
            best_ask = market_data.get('bestAsk', 0)
        ...
        # Get volume - prioritize cache data
        volume = float(market_data.get('volume', market_data.get('volumeNum', 0)))
        volume_24h = float(market_data.get('volume24hr', 0))
        if volume_24h == 0 and volume > 0:
            volume_24h = volume * 0.1  # Estimate 10% of total as 24h
        
        # Format as event structure
        return {
            "title": question,
            "markets": [{
                "question": question,
                ...
                "volumeNum": volume,
                "liquidityNum": float(market_data.get('liquidityNum', 0))
            }]
        }
