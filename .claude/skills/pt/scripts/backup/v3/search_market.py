...
def search_markets(query: str, limit: int = 10):
    """
    搜尋 Polymarket 市場（使用優化的服務器端搜尋）
    Search Polymarket markets (using optimized server-side search)
    ...
    """
    # 使用優化的搜尋版本
    return search_markets_optimized(query, limit)


def search_markets_optimized(query: str, limit: int = 10):
    """
    使用 Gamma API 的 public-search 端點進行全站搜尋
    Search Polymarket markets using server-side search (Gamma API public-search endpoint)
    ...
    try:
        print(f"\n🔍 使用服務器端搜尋: \"{query}\"")
        print(f"   Using server-side search: \"{query}\"\n")

        # Strategy 1: 使用專門的搜尋 endpoint (Server-side search)
        print(f"🚀 正在使用 Gamma public-search API...")
        search_url = f"{GAMMA_API}/public-search"

        params = {
            "q": query,           # 搜尋關鍵字
            "limit": limit,       # 返回數量
            "type": "event"       # 指定只搜尋 Event (過濾掉 tags/profiles)
        }
        ...
        if search_response.status_code == 200:
            ...
            if isinstance(search_results, list) and search_results:
                ...
                for result in search_results:
                    # Search results 可能包含 events 或直接是 markets
                    if 'markets' in result:
                        ...
                        all_markets.append({
                            ...
                            'condition_id': market.get('conditionId', 'N/A'),
                            'end_date_iso': market.get('endDate', 'N/A'),
                ...
                # 顯示結果
                print_results(query, filtered_markets, is_optimized=True)
                return filtered_markets

            else:
                print(f"⚠️  搜尋結果為空，嘗試備用方案...")
        else:
            print(f"⚠️  搜尋 API 返回 {search_response.status_code}，嘗試備用方案...")

    except Exception as e:
        print(f"⚠️  搜尋時出錯: {e}，使用備用方案...")

    # Fallback: 如果 search API 失敗，回退到原本的方法
    print(f"\n🔄 使用備用搜尋方案...")
    return search_markets_legacy(query, limit)


def print_results(query: str, markets, is_optimized: bool = False):
    """
    統一的結果顯示函數
    Unified result display function
    """
    if not markets:
        print(f"❌ 沒有找到匹配 \"{query}\" 的市場 / No markets found matching \"{query}\"")
        return

    method_text = "(服務器端搜尋)" if is_optimized else "(客戶端過濾)"
    print(f"✅ 找到 {len(markets)} 個市場 {method_text} / Found {len(markets)} markets {method_text}:\n")
    print("=" * 100)
    ...
    for idx, market in enumerate(markets, 1):
        ...
        # Display URL if slug is available
        if event_slug and event_slug != 'N/A':
...
def search_markets_legacy(query: str, limit: int = 10):
    """
    原本的搜尋方法（Client-side filtering） - 作為備用方案
    Legacy search method (Client-side filtering) - Used as fallback
    """
    try:
        print(f"\n🔍 使用客戶端過濾搜尋: \"{query}\"")
        print(f"   Using client-side filtering: \"{query}\"\n")
        ...
        print_results(query, filtered_markets, is_optimized=False)
        return filtered_markets
        ...
        if tokens:
            ...
            for token in tokens:
                ...
                try:
                    ...
                    # Handle different possible orderbook structures
                    asks = []
                    bids = []

                    if hasattr(ob, 'asks'):
                        asks = ob.asks
                    elif isinstance(ob, dict) and 'asks' in ob:
                        asks = ob['asks']

                    if hasattr(ob, 'bids'):
                        bids = ob.bids
                    elif isinstance(ob, dict) and 'bids' in ob:
                        bids = ob['bids']

                    # Try different ways to get prices - take LAST (highest) bid/ask
                    best_ask = None
                    best_bid = None

                    if asks:
                        try:
                            # Handle ask prices - take LAST (highest) ask
                            if hasattr(asks[-1], 'price'):
                                best_ask = float(asks[-1].price)
                            elif isinstance(asks[-1], dict) and 'price' in asks[-1]:
                                best_ask = float(asks[-1]['price'])
                            elif isinstance(asks[-1], (list, tuple)) and len(asks[-1]) > 1:
                                best_ask = float(asks[-1][1])  # Usually price is second element
                        except (IndexError, ValueError, KeyError):
                            pass  # Silently handle errors

                    if bids:
                        try:
                            # Handle bid prices - take LAST (highest) bid
                            if hasattr(bids[-1], 'price'):
                                best_bid = float(bids[-1].price)
                            elif isinstance(bids[-1], dict) and 'price' in bids[-1]:
                                best_bid = float(bids[-1]['price'])
                            elif isinstance(bids[-1], (list, tuple)) and len(bids[-1]) > 1:
                                best_bid = float(bids[-1][1])  # Usually price is second element
                        except (IndexError, ValueError, KeyError):
                            pass  # Silently handle errors
