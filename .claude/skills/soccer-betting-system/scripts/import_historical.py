...
INSERT = f"INSERT INTO historical_matches ({','.join(c[0] for c in CSV_MAP)}) VALUES ({PH})"
...
def main() -> None:
    ...
    total = 0
    for code, (league_key, name) in LEAGUES.items():
        for season_code, season in (('2425', '2024-25'), ('2526', '2025-26')):
            url = f'{BASE}/{season_code}/{code}.csv'
            ...
            count = 0
            for row in rows:
                if not (row.get('FTR') or '').strip():
                    continue
                values = []
                for col_name, csv_key in CSV_MAP:
                    if col_name == 'league_key':
                        values.append(league_key)
                    elif col_name == 'season':
                        values.append(season)
                    else:
                        values.append(_to_num(row.get(csv_key)))
                con.execute(INSERT, values)
                count += 1
            con.commit()
            total += count
            print(f'✅ {name:<18} {season} {count:>4} matches')
