/**
 * AI Tools Board Member Session — 與 AI 工具委員會個別成員一對一深度對話
 *
 * 從 ai-tools-board config.yaml 讀取成員列表，讓用戶選擇單一成員進行
 * 一對一會話。成員有完整的 read/write/edit/bash 工具，可透過聊天更新自己的
 * 個人知識庫（.pi/ai-tools-board/agents/<name>/<name>-knowledge.md）。
 *
 * Commands:
 ...
 *
 * Usage: pi -e extensions/boards/ai-tools-board-member-session.ts
 */
...
export default function (pi: ExtensionAPI) {
  ...
  function showRosterWidget(ctx: ExtensionContext): void {
    ...
    ctx.ui.setWidget(
      ...
      (_tui, theme) => {
        const lines: string[] = [];
        lines.push(theme.fg("accent", theme.bold("  AI Tools Board  ")) + theme.fg("dim", `(${boardConfig!.board.length} 位成員)`));
  ...
  function renderFooter(tui: any, theme: any, footerData: any) {
    return {
      ...
      render(width: number): string[] {
        const prefix = theme.fg("dim", "[ AI Tools Board · Member ]  ");
        if (!activeMember) {
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    const configPath = join(ctx.cwd, ".pi/ai-tools-board/config.yaml");
    if (existsSync(configPath)) {
      boardConfig = parseBoardConfigYaml(readFileSync(configPath, "utf-8"));
    } else {
      ctx.ui.notify("找不到 .pi/ai-tools-board/config.yaml", "warning");
    }
...
`;
  ...
  pi.registerCommand("member-select", {
    ...
    handler: async (args, ctx) => {
      if (!boardConfig) { ctx.ui.notify("委員會設定未載入", "error"); return; }
      ...
      const choice = await ctx.ui.select("選擇 AI Tools Board 成員", options);
